...
const ClassInCharge = ({selectedYearID}) => {
    const navigate = useNavigate();
    const {data, isLoading, isError, error} = useFacultyGetClassInchargesQuery(selectedYearID);
    if(isLoading){
        return (
        ...
        return (
            <>
                    {
            data?.inChargeClasses.length !==0 ?
             (
                <div className="p-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {
                    data?.inChargeClasses.map((classes, index) =>
                    <div className="max-w-xs relative group" key={index}>
                    <Card>
                      ...
                      <div className="mt-2.5 mb-5 flex items-center">
                          <h5 className="font-semibold tracking-tight text-gray-900 dark:text-white">
                            ...
                            <div className="opacity-0 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition duration-300 group-hover:opacity-100">
                                <div
                                    className="rounded-lg bg-blue-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-800 hover:cursor-pointer focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                                    onClick={() => submitClassID(classes._id)}
                                >
                                    View Classes
                                </div>
                            </div>
              ...
              (
                ...
                You are not assigned any class incharges
              </h5>   
            )
        }
            </>
