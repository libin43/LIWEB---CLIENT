...
const FacultyModal = (props) => {
    ...
    const handleSubmit = () => {
        ...
        navigate(`/faculty/exam_mark_input/students?className=${selectedClass.className}&classId=${selectedClass.classID}&subjectId=${selectedSubjectID}&examId=${selectedExamID}`)
        
    }
    const handleModal = () => {
        closeModal();
    }
    const {data, isLoading, isError, error} = useFacultyGetExamConductedClassesQuery(selectedSubjectID);
                ...
                <div className="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
                    ...
                    <div className="modal-content py-4 text-left px-6">
                        ...
                        <div className="my-5">
                            {data.classes.map((classes, index) =>
                                <label key={classes._id} className="inline-flex items-center mr-5">
                                    <input type="checkbox" className="form-checkbox" checked={selectedClass.classID === classes._id} onChange={() => handleClassSelect({classID: classes._id, className: classes.className})} />
                        ...
                        <div className="flex justify-end pt-2">
                            ...
                            <button className={`px-4 bg-cyan-700 p-3 rounded-lg text-white hover:bg-cyan-950 ${selectedClass.classID ? '' : 'opacity-50 cursor-not-allowed'}`} onClick={handleSubmit} disabled={!selectedClass.classID}>Enter Marks</button>
                        </div>
                    </div>
                </div>
            </div>
